...
# "Less is more: An Item Response Theory approach for shortening tests"
# To change the distribution of the theta of the respondents, the corresponding 
# R code needs to be uncommented
# To change the distribution of high discriminative items along the trait, 
# the desired setting for the simulation of the item discrimination parameters 
# needs to be uncommented
# As it is, the code simulates the data for 1000 persons with normally 
# distributed thetas and 100 items with uniform difficulty parameters and 
# discrimination parameters equally spread along the trait
...
# a = dchisq(abs(b[order(b)]),2)*4.3 # item discrimination parameters (NOT unifor
# ly distributed)
...
lab_item <- 1:ncol(data) # item labels
num_item <- seq(10, 90, by = 20)  # number of items included in the short form(s). This 
# can be either a vector of values or a single value. In the first case, 
# short forms composed of different numbers of items will be obtained, in the 
# second one, only one short form will be obtained with a number of items equal to 
# that indicated in the vector "num_item". The number of items in "num_item" must
